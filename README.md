...
- 필드에서 예상치 못한 모델이 좋은 성과를 보일때도 있어 실무에서의 내용을 잘 접목시킴
- 다양한 LLM을 직접 케이스에 따라 적용해보며 장단점을 비교해본 점이 좋았으며, 여러개의 모델을 결합해서 함께 피드백을 주는 형태로 디벨롭 가능성
- 튜닝시킨 모델을 허깅페이스 업로드 혹은 streamlit을 베포하여 실제 서비스 피드백을 받는것을 추천
